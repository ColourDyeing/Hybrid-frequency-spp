...
#else
/* Do not force dllimport when building the library itself or when
   WIN_DLL is not defined. Consumer can define EXPORT explicitly if
   dllimport is required. */
#define EXPORT
#endif
#else
#define EXPORT
...
/* MSVC uses __declspec(thread) (no leading underscore) */
...
EXPORT int  readdcb(const char *file, nav_t *nav, const sta_t *sta, gtime_t time);
